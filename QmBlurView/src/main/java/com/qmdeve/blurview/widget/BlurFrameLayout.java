...
public class BlurFrameLayout extends FrameLayout {
    ...
    private void createOptimizedG3RoundedRectPath(RectF rect, float radius, Path path) {
        ...
        float c = radius * 0.551915f;

        path.moveTo(rect.left + radius, rect.top);
        path.lineTo(rect.right - radius, rect.top);
        path.cubicTo(rect.right - radius + c, rect.top, rect.right, rect.top + radius - c, rect.right, rect.top + radius);
        path.lineTo(rect.right, rect.bottom - radius);
        path.cubicTo(rect.right, rect.bottom - radius + c, rect.right - radius + c, rect.bottom, rect.right - radius, rect.bottom);
        path.lineTo(rect.left + radius, rect.bottom);
        path.cubicTo(rect.left + radius - c, rect.bottom, rect.left, rect.bottom - radius + c, rect.left, rect.bottom - radius);
        path.lineTo(rect.left, rect.top + radius);
        path.cubicTo(rect.left, rect.top + radius - c, rect.left + radius - c, rect.top, rect.left + radius, rect.top);
        path.close();
    }
